    ...
    return {
        ...
        controller: ["$sce", function (a) {
            ...
            b.rows || (b.rows = [
                ...
                b.getText = function (b) {
                    if ("margin" === b.type) return "";
                    if ("shift" === b.type) return "&dArr;";
                    var c = b.text || b;
        ...
        link: function (c, d, e) {
            ...
            d.bind("contextmenu", function (a) {
                ...
                f.isUpperCase = !1, f.lastInputCtrl = null, f.startPos = null, f.endPos = null,
                f.printKeyStroke = function (a) {
                    if (f.lastInputCtrl) {
                        var b = angular.element(a.target || a.srcElement);
                        if (b.hasClass("erase")) return void f.eraseKeyStroke();
                        if (b.hasClass("shift")) return void f.inverseCase();
                        var c = angular.element(f.lastInputCtrl),
                            d = c.val(),
                            e = d.substring(0, f.startPos),
                            g = d.substring(f.endPos, d.length);
                        c.val(e + b.text() + g), c.triggerHandler("change"), f.startPos++ , f.endPos++ , f.setKeyboardLayout();
                    }
                },
                ...
                f.refocus = function () { f.lastInputCtrl.focus() },
                f.eraseKeyStroke = function () {
                    if (f.lastInputCtrl) {
                        ...
                        b.val(d + e), b.triggerHandler("change"), a ? f.endPos = f.startPos : (f.startPos-- , f.endPos--), f.lastInputCtrl.selectionStart = f.startPos, f.lastInputCtrl.selectionEnd = f.startPos, f.setKeyboardLayout(), f.refocus();
                    }
                }, f.setKeyboardLayout = function () {
                    return f.lastInputCtrl ? void (f.lastInputCtrl.className && f.isUpperCase ? f.inverseCase() : 0 === angular.element(f.lastInputCtrl).val().length ? f.isUpperCase || (f.isUpperCase = !1, f.inverseCase()) : " " === angular.element(f.lastInputCtrl).val().slice(-1) && !f.isUpperCase && e.hasOwnProperty("uppercaseAllWords") ? f.inverseCase() : (f.isUpperCase = !0, f.inverseCase())) : (f.isUpperCase = !1, void f.inverseCase());
                },
                b.find("input").bind("blur focus", function () {
                    f.setKeyboardLayout(), f.lastInputCtrl = this, f.lastInputCtrl && "checkbox" !== f.lastInputCtrl.type && (f.startPos = f.lastInputCtrl.selectionStart, f.endPos = f.lastInputCtrl.selectionEnd);
                })
                    .bind("keydown", function () {
                        f.startPos = f.lastInputCtrl.selectionStart, f.endPos = f.lastInputCtrl.selectionEnd;
                    }),
                a(function () {
                    f.inverseCase();
                }, 0);
